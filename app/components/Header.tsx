...
const Header = () => {
    ...
    return (
        <div 
            ...
            {/* Random Facts  */}
            <div>
                <p className={`${isFactVisible ? "" : "opacity-0"} transition-opacity duration-500 ease-in-out`}>  {facts[index]?.fact || "Loading facts..."} {facts[index]?.source ? `- ${facts[index].source}` : ""} </p>
            </div>
        </div>
    )
}
